...
Antes de iniciar a geração da matriz B, confirme a disponibilidade dos componentes abaixo.

| Componente | Finalidade | Documento ou seção de referência |
| --- | --- | --- |
| Ambiente Python `mpaswf` | Fornece os comandos `mpaswf`, `mpasbflow` e `mpasbcov`, além das dependências Python do workflow. | [Instalação do ambiente `mpaswf`](install-mpaswf-jaci.md) |
| WPS / `ungrib.exe` | Converte dados GFS em GRIB para arquivos intermediários usados na preparação das condições iniciais MPAS. Necessário apenas quando as amostras NMC forem produzidas a partir de GFS/GRIB. | [Instalação do WPS/ungrib](install-wps-ungrib-jaci.md) |
| MPAS-JEDI compilado | Disponibiliza `mpas_init_atmosphere`, `mpas_atmosphere`, `mpasjedi_error_covariance_toolbox.x` e `mpasjedi_variational.x`. | Documentação do MONAN-JEDI |
| Malha, partições e arquivos estáticos | Definem a geometria MPAS, a decomposição MPI, os campos invariantes, namelists, streams e tabelas físicas. | Seção 4 deste documento |

> Antes de executar `mpaswf nmc range`, valide o WPS/ungrib quando a fonte de dados for GFS em formato GRIB. Caso as previsões f24 e f48, ou as diferenças NMC, já estejam disponíveis, essa etapa pode ser dispensada.
...
#### 4.3.1. Onde devem estar antes da rodada
...
#### 4.3.2. Observação importante sobre `stream_list.atmosphere.control`
...
Antes de iniciar, confirme que o ambiente `mpaswf` está ativo. Quando a fonte for GFS/GRIB, confirme também que o WPS/ungrib foi validado conforme o documento de pré-requisitos.

Entre no repositório e prepare a sessão:
...
```text
1. Instalar e validar o ambiente Python `mpaswf`
2. Instalar e validar o WPS/ungrib quando as amostras NMC forem geradas a partir de GFS/GRIB
3. Garantir arquivos estáticos da malha, namelists, streams e tabelas físicas
4. Gerar pares NMC f48 - f24
5. Preparar BFLOW e o manifesto de amostras
6. Rodar VBAL para calibrar o balanço vertical e multivariado
7. Rodar HDIAG para calcular desvio padrão e escalas de correlação
8. Rodar NICAS para construir o operador de correlação
9. Rodar SO para testar a B dentro do 3DVar
10. Rodar DIRAC para verificar a resposta da B completa
11. Usar NICAS + StdDev + VBAL na configuração SABER do MPAS-JEDI
```
