#!/usr/bin/env bash
#BOP
# !ROUTINE: 1_download_wps_assets.sh
# !DESCRIPTION:
#   Downloads, validates and extracts the WPS source archive and the selected
#   WPS geographical static-data package using the portable installation layout.
# !INTERFACE:
#   bash scripts/wps/1_download_wps_assets.sh
# !SEE ALSO:
#   _common.sh, 2_probe_wps_build_environment.sh and 3_build_wps_ungrib.sh.
#EOP
set -euo pipefail
...
echo "WPS_GEOG_DIR=${GEOG_DIR}"
echo "Próximo passo: source scripts/load_jaci_env.sh && bash scripts/wps/2_probe_wps_build_environment.sh"
