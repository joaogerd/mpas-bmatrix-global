...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="mpashdiag", description="Prepara, submete e valida a etapa HDIAG")
    sub = p.add_subparsers(dest="cmd", required=True)

    prep = sub.add_parser("prepare")
    prep.add_argument("--config", default=DEFAULT_CONFIG)
    prep.add_argument("--vbal-workspace", required=True)
...
def prepare_command(args) -> int:
    ...
    prepare(config, args.vbal_workspace, workspace=args.workspace, clean=args.clean)
    return 0
